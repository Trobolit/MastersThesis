...
\begin{itemize}
    \item Talk about the logging issues, hard faults and time slowdown here or in conclusion.
\end{itemize}
...
\item SOMEWHERE present the intended operating frequencies / sampling time
%\item Wobbling/small oscillations in step responses when flying most likely due to hinge play in elevons. They got worse after a few crashes and are also under higher pressure. They may also correspond to the self oscillations in the servos we've seen when they are pushed a little bit too hard on PWM frequency.
\item Maybe add link to matlab/simulink code as reference?
\end{itemize}
